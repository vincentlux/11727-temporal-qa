...
class McTacoEvaluator:
    ...
    def __init__(self, test_file=None, output_file=None, output=None):
        self.test_file = test_file
        self.output_file = output_file
        self.output = output

    def get_result(self, ref_lines = None, prediction_lines = None):

        ref_lines = [x.strip() for x in open(self.test_file).readlines()] if not ref_lines else ref_lines
        prediction_lines = [x.strip() for x in open(self.output_file).readlines()] if not prediction_lines else prediction_lines
        ...
        for question in result_map:
            ...
            if p + r > 0.0:
                f1 += 2 * p * r / (p + r)
        return correct, f1, total

    def get_em_f1(self, ref_lines=None, prediction_lines=None):
        correct, f1, total = self.get_result(ref_lines, prediction_lines)
    ...
    def print_result(self):
        correct, f1, total = self.get_result()
        print("Strict Acc.: " + str(correct / total))
        print("Avg F1: " + str(f1 / total))
